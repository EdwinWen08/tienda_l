...
public class RegistroService {
    ...
    //Se ejecuta cuando llega correo y da clic a enlace
    public Model activar(Model model, String username, String clave) {
        Usuario usuario = usuarioService.getUsuarioPorUsernameYPassword(username, clave); //Busca si hay usuario y password temporal
        if (usuario != null) { //Si estaba...
            model.addAttribute("usuario", usuario);
        } else { //Hay que devolver error
            model.addAttribute("titulo", messageSource.getMessage("registro.activar", null, Locale.getDefault())); //Ponga de titulo esto de messages
            model.addAttribute("mensaje", messageSource.getMessage("registro.activar.error", null, Locale.getDefault()));
        }
        return model;
    }
    
    //El que finalmente crea el usuario en sistema
    public void activar(Usuario usuario, MultipartFile imagenFile) {
        var codigo = new BCryptPasswordEncoder(); //Crear claves seguras
        
        usuario.setPassword(codigo.encode(usuario.getPassword()));
        
        if (!imagenFile.isEmpty()) {
            usuarioService.save(usuario, false);
            String ruta = firebaseStorageService.cargaImagen(imagenFile, "usuario", usuario.getIdUsuario());
